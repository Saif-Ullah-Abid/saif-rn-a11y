...
export function checkMissingLabel(nodes: JsxNodeInfo[]): Finding[] {
  const findings: Finding[] = [];

  for (const node of nodes) {
    if (!INTERACTIVE_TAGS.has(node.tagName)) continue;

    const isIconOnly = node.hasNonTextChild && !node.textContent;
    if (!isIconOnly) continue;

    const hasLabel =
      "accessibilityLabel" in node.attributes || "aria-label" in node.attributes;
